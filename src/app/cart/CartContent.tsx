...
export function CartContent({ initialItems }: { initialItems: any[] }) {
  const router = useRouter()
  const [items, setItems] = useState<any[]>(initialItems)
  const [selected, setSelected] = useState<Set<string>>(new Set(initialItems.map(i => i.product_id)))
  ...
  const totalPrice = origTotal - discTotal
          ...
          <>
            <div style={{ background: 'white', border: '1px solid #E8E4DD', borderRadius: 12, overflow: 'hidden', marginBottom: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 24px', borderBottom: '1px solid #E8E4DD', background: '#F8F6F2' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: PRETENDARD, fontSize: 13, fontWeight: 600, color: C.charcoal, cursor: 'pointer' }}>
                  <input type="checkbox"
                    ...
                    onChange={e => setSelected(e.target.checked ? new Set<string>(items.map(i => i.product_id)) : new Set<string>())}
                  />
              ...
              {items.map(item => {
                ...
                return (
                  <div key={item.product_id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 24px', borderBottom: '1px solid #F0EDE8', background: isSelected ? 'white' : '#FAFAF8' }}>
                    ...
                    <p style={{ fontFamily: PRETENDARD, fontSize: 15, fontWeight: 700, color: C.charcoal, textAlign: 'right' }}>
            ...
            <div style={{ background: 'white', border: '1px solid #E8E4DD', borderRadius: 12, padding: 24 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ fontFamily: PRETENDARD, fontSize: 14, color: C.charcoal }}>0원</span>
                </div>
              </div>
